...
# Ensure Python can locate the `python-package` directory

#sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'python-package')))
#project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
#python_package_path = os.path.join(project_root, "python-package")
...
from employee_events.employee import QueryBase, Employee
#from python_package.employee_events.employee import QueryBase, Employee
...
class BarChart(MatplotlibViz):
    ...
    def visualization(self, model, asset_id):
        ...
        if model.name == "team":
            pred = proba.mean()  # Mean for teams
            
        # Otherwise set `pred` to the first value
        # of the predict_proba output

        else:
            pred = proba[0]  # First value for individuals
...
class DashboardFilters(FormGroup):
    ...
    children = [
        ...
        Button(text="Submit", type="submit")  # <-- Ajout du bouton
    ]
...
async def update_data(r: Request):
    data = await r.form()
